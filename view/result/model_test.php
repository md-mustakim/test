	...
	else
	{
		...
		{	
			...
			else
			{
				...
				if($type==1){$semester=100; $pre_semester=101; $weekly=111; $wm='1st semester weekly'; $sm='2nd Model'; $p_sm='1st Pre Semester';}
				if($type==2){$semester=200; $pre_semester=201; $weekly=211; $wm='2nd semester weekly'; $sm='2nd Model'; $p_sm='2nd Pre Semester';}
				if($type==3){$semester=300; $pre_semester=301; $weekly=311; $wm='3rd semester weekly'; $sm='3rd Model'; $p_sm='3rd Pre Semester';}
				...
				while($stc= mysqli_fetch_assoc($get_stc))
				{
					...
					if($pre_status==1){$total= $semester_marks;
					}else {
						$total= $semester_marks;
					}
					
				        $gpa=grade($total);$point= gpa($total);
						if($point==0){$failed=1;} 
				...
				<tr>
					...
					if($testdata==0){
						$outputd= "<div align='center'><b><span style='font-size:20px;'><form action='model_test.php?regid=$unid&sem=$type' method='POST'><input type='submit' name='final' value='final submit' title='this action cannot be undo so carefull' /></form></span></b></div>";
						}else {$outputd='';}
						...
						if($testdata==0) {
							...
							if(!$cca)
							{
								...
								if (window.confirm('Successfully confermed!! wait 2s;'))
													{
														 window.location.href = 'model_test.php?regid=$unid&sem=$type';
													}
													else
													{
														window.location.href = 'model_test.php?regid=$unid&sem=$type';
...
<div class='print' ><br />
	...
	<div  align='right' style='float:left;width:100%;margin:0;padding:0;outline:0;'>
