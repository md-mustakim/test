		...
		{	
			...
			else
			{
				...
				while($stc= mysqli_fetch_assoc($get_stc))
				{
					...
					$p_m= mysqli_fetch_assoc(mysqli_query($new,"SELECT * FROM marksheed_2018 where subject_id=$subjid and regid=$unid and type=$pre_semester"));
					$s_m= mysqli_fetch_assoc(mysqli_query($new,"SELECT * FROM marksheed_2018 where subject_id=$subjid and regid=$unid and type=$semester"));
