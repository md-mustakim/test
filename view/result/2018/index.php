<?php 
	...
	else
	{
		...
		while($r=mysqli_fetch_assoc($result_check_list))
		{
			...
			//$unid= $r[''];
			$first_sm= mysqli_num_rows(mysqli_query($new,"SELECT * FROM marksheed_2018 where regid=$unid and type=100"));
			$first_sm_p= mysqli_num_rows(mysqli_query($new,"SELECT * FROM marksheed_2018 where regid=$unid and type=101"));
			$secnond_sm= mysqli_num_rows(mysqli_query($new,"SELECT * FROM marksheed_2018 where regid=$unid and type=200"));
			$secnond_sm_p= mysqli_num_rows(mysqli_query($new,"SELECT * FROM marksheed_2018 where regid=$unid and type=201"));
			$third_sm= mysqli_num_rows(mysqli_query($new,"SELECT * FROM marksheed_2018 where regid=$unid and type=300"));
			$third_sm_p= mysqli_num_rows(mysqli_query($new,"SELECT * FROM marksheed_2018 where regid=$unid and type=301"));
...
<body>
	...
	<div align='center' class="column3"><a align='left' class="bttn" href="setsubj.php">Costomize Subject</a></div>
</body>
</html>